...
public class UserServices {
	...
	public UserDto registerUser(UserDto userDto) {
		...
		user.setAccountStatus(AccountStatus.builder().accountStatusId(1).statusId(0) // 0 = inactive
				.token(UUID.randomUUID().toString()).createdAt(LocalDateTime.now()).tokenType("verification")
		...
		if (Objects.nonNull(savedUser)) {
			String verificationLink = frontendBaseUrl + "/verify?token=" + savedUser.getAccountStatus().getToken();
	...
	public User generateAndSendNewPassword(String email) {
		...
		try {
			String htmlContent = templateEngine.process("email/forgot_password", context); // must exist in
