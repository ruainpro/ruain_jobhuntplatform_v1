...
public class UserController {
	...
	public ResponseEntity<ApiResponse<AuthResponse>> login(@RequestBody AuthRequest request,
		...
		try {
			...
			String token = jwtService.generateTokenWithClaims(user.getEmail(),
	...
	public ResponseEntity<ApiResponse<String>> logout(HttpServletResponse response) {
        ...
        actionHistoryServices.addActionHistory(jwtService.getPublicIdFromCurrentRequest(),  ":logout");
		ResponseCookie clearedCookie = ResponseCookie.from("jwt", "").httpOnly(true).secure(true).path("/").maxAge(0)
				.build();
