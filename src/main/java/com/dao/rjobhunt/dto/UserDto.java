...
public class UserDto {
	...
	public static UserDto fromEntity(User user, AccountStatus status) {
		return new UserDto(user.getPublicId(), user.getEmail(), user.getPassword(), user.getPhoneNumber(),
	...
	public User toEntity() {
		return User.builder().publicId(this.publicId != null ? this.publicId : UUID.randomUUID()).email(this.email)
				.phoneNumber(this.phoneNumber).gender(this.gender).dateOfBirth(this.dateOfBirth).address(this.address)
				.role(this.role).createdAt(this.createdAt != null ? this.createdAt : LocalDateTime.now())
				.updatedAt(LocalDateTime.now()).preferredJobTitles(this.preferredJobTitles).build();
	}
}
